...
public class ReleaseChecks<V extends Comparable<? super V>> {
    ...
    public ReleaseChecks<V> noUncommittedChanges() {
        ...
        if (!uncommited.isEmpty()) {
            describeProblem(b -> b.severity(Severity.PROBLEM)
    ...
    public ReleaseChecks<V> projectNotAlreadyReleased() {
        ...
        final GAVCDescriptor releaseDescriptor = DefaultGAVCDescriptor.builder()
                .group(getDescriptor().getGroup())
                .artifact(getDescriptor().getArtifact())
        ...
        Optional.ofNullable(releasedArtifact).ifPresent(r ->
                describeProblem(b -> b.severity(Severity.PROBLEM)
                        ...
                        .formatArgs(releaseVersion, getDescriptor().getGroup(), getDescriptor().getArtifact()))
        );
    ...
    public ReleaseChecks<V> projectReferencesNoSnapshotDependencies() {
        ...
        getDependencyContainers().forEach(container -> {
    ...
    public ReleaseChecks<V> releaseVersionMatchesAtLeastApiCompatibility() {
        ...
        final GAVCDescriptor releaseDescriptor = DefaultGAVCDescriptor.builder()
                .group(getDescriptor().getGroup()).artifact(getDescriptor().getArtifact()).version(currentReleaseVersion.toString()).build();

        final ResolvedArtifact lastRelease = getArtifactResolver().resolveLastReleaseArtifact(releaseDescriptor);
    ...
    private V getParsedVersion() {
        return getVersionHandler().parse(getDescriptor().getVersion(), getProjectDir());
    }

    @Nullable
    private ApiCompatibilityReport createApiCompatibilityReport(@Nonnull final ResolvedArtifact baseLineArtifact) {

        final Set<File> baseLineClasspath = collectFiles(baseLineArtifact.getDependencies());
        final Set<File> currentClasspath = collectFiles(getDependencies());
